...
const baseSize = 512;
const ProgressRadial = create_ssr_component(
...
<figure class="${
      'progress-radial ' + escape(classesBase, true)
    }" data-testid="progress-radial" role="meter"${add_attribute(
      'aria-labelledby',
      labelledby,
      0,
    )}${add_attribute('aria-valuenow', value || 0, 0)}${add_attribute(
      'aria-valuetext',
      value ? `${value}%` : 'Indeterminate Spinner',
      0,
    )}${add_attribute('aria-valuemin', 0, 0)}${add_attribute(
      'aria-valuemax',
      100,
      0,
    )}>
	<svg viewBox="${
    '0 0 ' + escape(baseSize, true) + ' ' + escape(baseSize, true)
  }" class="${['rounded-full', value === void 0 ? 'animate-spin' : '']
    .join(' ')
    .trim()}"><circle class="${
    'progress-radial-track ' +
    escape(cBaseTrack, true) +
    ' ' +
    escape(track, true)
  }"${add_attribute('stroke-width', stroke, 0)}${add_attribute(
    'r',
    baseSize / 2,
    0,
  )} cx="50%" cy="50%"></circle><circle class="${
    'progress-radial-meter ' +
    escape(cBaseMeter, true) +
    ' ' +
    escape(meter, true)
  }"${add_attribute('stroke-width', stroke, 0)}${add_attribute(
    'r',
    baseSize / 2,
    0,
  )} cx="50%" cy="50%"${add_styles({
    'stroke-dasharray': `${circumference}
			${circumference}`,
    'stroke-dashoffset': dashoffset,
  })}></circle>${
    value != void 0 && value >= 0 && $$slots.default
      ? `<text x="50%" y="50%" text-anchor="middle" dominant-baseline="middle" font-weight="bold"${add_attribute(
          'font-size',
          font,
          0,
        )} class="${'progress-radial-text ' + escape(fill, true)}">${
...
const cTransitionLayer =
  'w-full h-full p-4 overflow-y-auto flex justify-center';
const cModal = 'block';
...
const Modal = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_modalStore = subscribe(
    modalStore,
  ...
  modalStore.subscribe((modals) => {
    ...
    buttonTextCancel =
      modals[0].buttonTextCancel || buttonTextDefaults.buttonTextCancel;
    buttonTextConfirm =
      modals[0].buttonTextConfirm || buttonTextDefaults.buttonTextConfirm;
    buttonTextSubmit =
      modals[0].buttonTextSubmit || buttonTextDefaults.buttonTextSubmit;
    currentComponent =
  ...
  if (
    $$props.buttonNeutral === void 0 &&
    $$bindings.buttonNeutral &&
    buttonNeutral !== void 0
  )
    $$bindings.buttonNeutral(buttonNeutral);
  if (
    $$props.buttonPositive === void 0 &&
    $$bindings.buttonPositive &&
    buttonPositive !== void 0
  )
    $$bindings.buttonPositive(buttonPositive);
  if (
    $$props.buttonTextCancel === void 0 &&
    $$bindings.buttonTextCancel &&
    buttonTextCancel !== void 0
  )
    $$bindings.buttonTextCancel(buttonTextCancel);
  if (
    $$props.buttonTextConfirm === void 0 &&
    $$bindings.buttonTextConfirm &&
    buttonTextConfirm !== void 0
  )
    $$bindings.buttonTextConfirm(buttonTextConfirm);
  if (
    $$props.buttonTextSubmit === void 0 &&
    $$bindings.buttonTextSubmit &&
    buttonTextSubmit !== void 0
  )
    $$bindings.buttonTextSubmit(buttonTextSubmit);
  if (
    $$props.regionBackdrop === void 0 &&
    $$bindings.regionBackdrop &&
    regionBackdrop !== void 0
  )
    $$bindings.regionBackdrop(regionBackdrop);
  if (
    $$props.regionHeader === void 0 &&
    $$bindings.regionHeader &&
    regionHeader !== void 0
  )
    $$bindings.regionHeader(regionHeader);
  if (
    $$props.regionBody === void 0 &&
    $$bindings.regionBody &&
    regionBody !== void 0
  )
    $$bindings.regionBody(regionBody);
  if (
    $$props.regionFooter === void 0 &&
    $$bindings.regionFooter &&
    regionFooter !== void 0
  )
    $$bindings.regionFooter(regionFooter);
  cPosition = $modalStore[0]?.position ?? position;
  classesBackdrop = `${cBackdrop} ${regionBackdrop} ${zIndex} ${
    $$props.class ?? ''
...
${
  $modalStore.length > 0
    ? `
		<div class="${
      'modal-backdrop ' + escape(classesBackdrop, true)
    }" data-testid="modal-backdrop">
			<div class="${'modal-transition ' + escape(classesTransitionLayer, true)}">${
        $modalStore[0].type !== 'component'
          ? `
					<div class="${
            'modal ' + escape(classesModal, true)
          }" data-testid="modal" role="dialog" aria-modal="true"${add_attribute(
            'aria-label',
            $modalStore[0].title ?? '',
            0,
          )}>
              ...
              $modalStore[0]?.title
                ? `<header class="${
                    'modal-header ' + escape(regionHeader, true)
                  }"><!-- HTML_TAG_START -->${
                    $modalStore[0].title
                  }<!-- HTML_TAG_END --></header>`
                : ``
              ...
              $modalStore[0]?.body
                ? `<article class="${
                    'modal-body ' + escape(regionBody, true)
                  }"><!-- HTML_TAG_START -->${
                    $modalStore[0].body
                ...
                : ``
						...
						${
              $modalStore[0]?.image && typeof $modalStore[0]?.image === 'string'
                ? `<img class="${
                    'modal-image ' + escape(cModalImage, true)
                  }"${add_attribute(
                    'src',
                    $modalStore[0]?.image,
                    0,
              ...
              $modalStore[0].type === 'alert'
                ? `
							<footer class="${
                'modal-footer ' + escape(regionFooter, true)
              }"><button type="button" class="${
                'btn ' + escape(buttonNeutral, true)
              }">${escape(buttonTextCancel)}</button></footer>`
                : `${
                ...
                'btn ' + escape(buttonNeutral, true)
              }">${escape(buttonTextCancel)}</button>
								<button type="button" class="${'btn ' + escape(buttonPositive, true)}">${escape(
                  buttonTextConfirm,
                )}</button></footer>`
                      : `${
                ...
                {},
                  ...
                  'btn ' + escape(buttonNeutral, true)
									...
									<button type="submit" class="${'btn ' + escape(buttonPositive, true)}">${escape(
                    buttonTextSubmit,
                  )}</button></footer></form>`
                            : ``
                        }`
          ...
          : `
					...
					<div class="${
            'modal contents ' + escape($modalStore[0]?.modalClasses ?? '', true)
          }" data-testid="modal-component" role="dialog" aria-modal="true"${add_attribute(
            'aria-label',
            $modalStore[0].title ?? '',
            0,
          )}>${validate_component(
            currentComponent?.ref || missing_component,
            'svelte:component',
          ).$$render(
            $$result,
            Object.assign({}, currentComponent?.props, { parent }),
            {},
            {
              default: () => {
                return `${
                  currentComponent?.slot
                    ? `<!-- HTML_TAG_START -->${currentComponent?.slot}<!-- HTML_TAG_END -->`
                    : ``
                }`;
              },
            },
