import { c as create_ssr_component, s as setContext, e as escape, b as add_attribute, g as getContext, d as createEventDispatcher, f as compute_slots, v as validate_component, h as each } from "../../chunks/index3.js";
...
const TreeViewItem = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<details class="${"tree-item " + escape(classesBase, true)}" data-testid="tree-item"${add_attribute("open", open, 1)}><summary class="${"tree-item-summary " + escape(classesSummary, true)}" role="treeitem" aria-selected="false"${add_attribute("aria-expanded", $$slots.children ? open : void 0, 0)}>
		<div class="${"tree-summary-symbol " + escape(classesSymbol, true)}">${$$slots.children ? `
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M201.4 374.6c12.5 12.5 32.8 12.5 45.3 0l160-160c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L224 306.7 86.6 169.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3l160 160z"></path></svg>` : `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" class="${"w-3 " + escape(classesHyphen, true)}"><path d="M432 256c0 17.7-14.3 32-32 32L48 288c-17.7 0-32-14.3-32-32s14.3-32 32-32l352 0c17.7 0 32 14.3 32 32z"></path></svg>`}</div>
		
		${$$slots.lead ? `<div class="tree-item-lead">${slots.lead ? slots.lead({}) : ``}</div>` : ``}
		
		<div class="tree-item-content">${slots.default ? slots.default({}) : ``}</div></summary>
	<div class="${"tree-item-children " + escape(classesChildren, true)}" role="group">${slots.children ? slots.children({}) : ``}</div></details>`;
});
...
${validate_component(TreeView, "TreeView").$$render($$result, {}, {}, {
    default: () => {
      return `${validate_component(TreeViewItem, "TreeViewItem").$$render($$result, {}, {}, {
        children: () => {
          return `<p>nextボタンをおしたときに設定が保存されます</p>
      <p>とりあえずnprofileの方だけに適応</p>
      <p>(naddrのほうはデフォルトのまま)</p>
      ${validate_component(TreeViewItem, "TreeViewItem").$$render($$result, {}, {}, {
            children: () => {
              return `<button type="button" class="btn variant-filled-surface mb-3 mt-1">デフォルトに戻す
          </button>
          <div class="relay input-group input-group-divider grid-cols-[1fr_auto] h-12"><input class="px-2" type="text" placeholder="wss://..." ${""}${add_attribute("value", relay, 0)}>
            <button class="py-1 btn variant-filled">add relay</button></div>
          <ul class="border-solid border-2 border-surface-500/25 mx-8 my-1">リレーリスト
      ...
      ${validate_component(TreeViewItem, "TreeViewItem").$$render($$result, {}, {}, {
          ...
          <label class="flex items-center space-x-2"><input class="checkbox" type="checkbox"${add_attribute("checked", URLPreview, 1)}>
            <p>自動的に画像を読み込む、URLプレビューを表示する</p></label>
          <label class="flex items-center space-x-2"><input class="checkbox" type="checkbox"${add_attribute("checked", loadEvent, 1)}>
            <p>イベントの内容を自動で読み込む</p></label>
        `;
            },
            default: () => {
              return `軽量用設定
